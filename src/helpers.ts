...
export function useSplTokenBalances(
    walletAddress: string
): UseSplTokenBalancesReturn {
    ...
    useEffect(() => {
        ...
        async function fetchTokenBalances() {
            ...
            try {
                ...
                for (const { account } of tokenAccounts.value) {
                    ...
                    if (decimals === undefined) {
                        try {
                            ...
                            console.warn(
                                `Could not fetch decimals for mint ${mintAddress}:`,
                                e
                            );
                            continue;
                        }
                    ...
                    if (decimals === undefined) {
                        console.warn(
                            `Skipping token with undefined decimals: ${mintAddress}`
                        );
                        continue;
                    }

                    const uiAmount = accountInfo.amount
                    ...
                    if (uiAmount > 0) {
                        fetchedBalances.push({
                            ...
                            symbol:
                                tokenInfo?.symbol ||
                                mintAddress.substring(0, 4) +
                                    '...' +
                                    mintAddress.substring(
                ...
                setTokenBalances(
                    fetchedBalances.sort((a, b) => b.uiAmount - a.uiAmount)
                );
            } catch (err) {
                console.error(
                    `Failed to fetch token balances for ${walletAddress}:`,
                    err
                );
                if (err instanceof Error) {
                    setError(err.message);
                    ...
                    setError(
                        'An unknown error occurred while fetching token balances.'
                    );
                }
                setTokenBalances([]);
            } finally {
